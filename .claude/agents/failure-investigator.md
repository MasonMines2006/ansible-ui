...
description: >
  ...
  FIX PRODUCT / QUARANTINE / SKIP). With --fix, auto-applies the recommended fix.
  ...
  (quarantine via Currents API, update baselines, stabilize selectors, etc.).
  </commentary>
  </example>
model: inherit
color: red
...
|---------------|----------|
| **NEW** | Failure rate < 15% AND fewer than 3 failures, OR first failure within last 48 hours, OR test not found in history |
| **RECURRING** | Failure rate between 15% and 85% |
| **CHRONIC** | Failure rate > 85% with 10+ executions |

Edge case: If a test has fewer than 5 total executions and a high failure rate, classify as **NEW** — it hasn't been running long enough to be chronic.
...
3. Parse the ciBuildId to extract the topology using the same logic as pipeline-health:

   Extract topology by matching (case-insensitive) against the ciBuildId:

   | Substring | Display Name |
   |-----------|-------------|
   | `ocp-a` | OCP A |
   | `saas` | SaaS |
   | `cont-b` | Container B |
...
| `toHaveScreenshot`, `toMatchSnapshot`, `screenshot`, `pixel` | VISUAL |
...
Report:
- If a matching action exists: its type (quarantine/skip/tag), status (active/disabled), creation date, and description
- If no matching action: "No existing quarantine or skip rules found."
...
Map the failing test's spec path to a feature area:
...
For each PR, map its changed files to feature areas using the same table. Determine correlation:
...
**Error Type:** {TIMEOUT / ASSERTION / VISUAL / NETWORK / AUTH_RBAC / SERVER_ERROR / SELECTOR / OTHER}
...
```
No existing quarantine or skip rules found for this test.
```
...
|--------|------|--------|---------|-------------|
| {actionId} | quarantine | active | May 20 | {description} |
```
...
```
**Recommendation:** {FIX TEST / FIX PRODUCT / QUARANTINE / SKIP}
...
|-----------|---------------|
| VISUAL error type | **FIX TEST** — update baseline screenshots |
| CHRONIC + existing quarantine/skip action | **No action needed** — already handled |
| CHRONIC + no existing action + ALL_TOPOLOGIES | **QUARANTINE** — stop pipeline noise, fix separately |
...
| CHRONIC + ALL_TOPOLOGIES | **QUARANTINE** — known broken, not urgent unless blocking release |
...
### Strategy A: FIX TEST — Visual Baseline Update

**Trigger:** Remediation is FIX TEST and error type is VISUAL.

1. Identify the failing spec file from `specFilePath`.
2. Run the test locally to regenerate baselines:

   ```bash
   cd playwright && npx playwright test {specFilePath} --update-snapshots --project 'live chromium'
   ```

3. If the test passes, report: "Baselines updated. Review the diff in `playwright/tests/**/*-snapshots/` before committing."
4. If the test fails, report: "Baseline update failed — the test has non-visual issues too. Fix those first."

### Strategy B: FIX TEST — Timeout / Assertion Stabilization
...
### Strategy C: QUARANTINE — Create Currents Action

**Trigger:** Remediation is QUARANTINE.

1. Check if an existing quarantine action was already found in Step 7. If so, report: "Already quarantined (action {actionId}). No action needed." and stop.

2. Build the action name: `Quarantine: {spec short name} — {test title (truncated to 80 chars)}`
...
3. Call `currents-create-action` with:
   ...
   - `matcher`:
     ...
     ```
   - `description`: `Auto-quarantined by /pipeline-triage --fix on {YYYY-MM-DD}. Classification: {CHRONIC/RECURRING}. Failure rate: {rate}% over {DAYS}d.`

4. Report: "Quarantine action created (ID: {actionId}). The test will be quarantined in future runs."

### Strategy D: SKIP — Create Currents Skip Action

**Trigger:** Remediation is SKIP.

Same as Strategy C but with `"op": "skip"` instead of `"op": "quarantine"`, and action name prefix `Skip:` instead of `Quarantine:`.

### Strategy E: FIX PRODUCT — No Auto-Fix
...
### Strategy F: No Action Needed

**Trigger:** Remediation indicates no action is needed (e.g., already quarantined/skipped).
