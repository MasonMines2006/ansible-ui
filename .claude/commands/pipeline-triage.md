...
|----------------|-----------------|
| FIX TEST (visual) | Regenerate baseline screenshots with `--update-snapshots` |
| FIX TEST (timeout/assertion) | Stabilize selectors, add waits, or relax assertions in the spec file |
| QUARANTINE | Create a Currents quarantine action via the API |
...
- Existing quarantine/skip rules
- Correlated git commits/PRs around first failure date
- Remediation recommendation: FIX TEST / FIX PRODUCT / QUARANTINE / SKIP
